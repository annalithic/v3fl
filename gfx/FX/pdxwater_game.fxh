...
PixelShader =
{
	...
	Code
	[[
		float GetNightWaterAdjustment(float3 WorldSpacePos) {
			if(IsNeath(WorldSpacePos)) return AmbientPosY;
			return _NightWaterAdjustment;
		}
		...
		float3 GameCalcRefraction( float3 WorldSpacePos, float3 Normal, float2 ScreenPos, float3 WaterColor, float Depth, float NightWaterAdjustment )
		{
			float3 WaterColorMap = lerp( WaterColor, _WaterColorMapTint, _WaterColorMapTintFactor ) * NightWaterAdjustment;
				...
				float3 RefractionWaterColorMap = PdxTex2D( WaterColorTexture, RefractionWaterColorUV ).rgb * NightWaterAdjustment;
		...
		{
			float Night = GetNightWaterAdjustment(Input._WorldSpacePos);
			...
			// Refraction
			float3 Refraction = GameCalcRefraction( Input._WorldSpacePos, Normal, Input._ScreenSpacePos.xy, WaterColorAndSpec.rgb, Input._Depth, Night );
			...
			// Cubemap reflection
			float3 Reflection = CalcReflection( Normal, ToCameraDir ) * Night;
