			...
			PDX_MAIN
			{
				...
				SLightingProperties LightingProps = GetSunLightingProperties( Input.WorldSpacePos, ShadowMap );
						...
						// Second sun
							...
							if(IsNeath(Input.WorldSpacePos)) {
								SecondLightingProps._LightIntensity *= AmbientPosX;
								SecondLightingProps._CubemapIntensity *= AmbientPosY;
							}
				...
				// Nighttime lights and visuals
					...
					float DayNightModifier = IsNeath(Input.WorldSpacePos) ? 1 : Remap( _DayNightValue, _LightsActivateBegin, _LightsActivateEnd, 0.0, 1.0 );
